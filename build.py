...
fpm_common_args = "-s dir --log error  --vendor {} --url {} --after-install {} --license {} --maintainer {} --name telegraf --config-files {} --config-files {} --description \"{}\"".format(VENDOR, PACKAGE_URL, POSTINST_SCRIPT, PACKAGE_LICENSE, MAINTAINER, CONFIG_DIR, LOGROTATE_DIR, DESCRIPTION)
...
def build(version=None,
    ...
    for b, c in targets.iteritems():
        ...
        build_command += "{}/godep go build -o {} ".format(os.environ.get("GOBIN"), b)
        if race:
            build_command += "-race "
...
def package_scripts(build_root):
    ...
    shutil.copyfile(LOGROTATE_SCRIPT, os.path.join(build_root, LOGROTATE_DIR[1:], LOGROTATE_SCRIPT.split('/')[1]))
